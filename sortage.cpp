...
int bubble(float mas[], int len_mas, long int* p_count_comp, long int* p_count_perm);
void ckoarrr(float mas[], int l, int r, long int* p_count_comp, long int* p_count_perm);
int middle(float mas[], int l, int r, long int* p_count_comp, long int* p_count_perm);
void insert_sort(float mas[], int len_mas, long int* p_count_comp, long int* p_count_perm);
void shift(float mas[], int first_el, int last_el);
void shell(int array[], int size, long int* p_count_comp, long int* p_count_perm);
void sort_menu(float mas[], int len_mas, long int* p_count_comp, long int* p_count_perm, clock_t* start, clock_t* end);
...
int main()
{
    ...
    int len_mas = 0;
    long int count_comp = 0;
    long int* p_count_comp = &count_comp;
    long int count_perm = 0;
    long int* p_count_perm = &count_perm;
    clock_t start, end;
    double time;
    int flag = 1;
    int i;
    ...
    while (flag)
    {
        ...
        case 1:
            ...
            printf_s("\nПоздравляю, Вы успешно добавили массив: ");
            check_result(dmas, len_mas);
            break;
        case 2:
            if (len_mas != 0)
            {
                ...
                printf_s("Время выполнения сортировки: %e \n", time);
                count_comp = 0;
                count_perm = 0;
                for (i = 0; i < len_mas; i++)
                {
...
int middle(float mas[], int l, int r, long int* p_count_comp, long int* p_count_perm)
{
    ...
    while (true)
    {
        while (mas[l] < main_element) { l++; *p_count_comp++; }
        while (mas[r] > main_element) { r--; *p_count_comp++; }
...
void shift(float mas[], int first_el, int last_el, long int* p_count_comp)
{
    int i;
...
void shell(float mas[], int len_mas, long int* p_count_comp, long int* p_count_perm)
{
    int step;
    int i, j;
