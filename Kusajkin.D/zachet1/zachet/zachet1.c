...
char* menu[MENU_SIZE] = { "Заполнить массивы новыми данными (0)",  "Сортировка Пузырьком (1)", "Сортировка Выбором (2)", "Сортировка Хоара (3)", "Поразрядная сортировка (4)", "Изменить размер массива (5)", "Копировать массив А в массивы В и С (6)", "Сравнить правильность выполненной сортировки (7)", "Выход (8)"};
...
int main()
{
	...
	while (flag != 0)
	{
		...
		case 4: {minus(mas_a, dlin, &k); 
			...
			seal(menu); 
			break; }
		case 5: {scanf_s("%d", &dlin); 
			seal(menu); 
			break; }
		case 6: {copy(mas_a, mas_b, dlin); 
			copy(mas_a, mas_c, dlin); 
			seal(menu); 
			break; }
		case 7: {if (check_mas(mas_c, mas_b, dlin))
		{
